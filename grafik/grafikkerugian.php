...
while($data = $rugi->fetch_assoc()){
  ...
  $index_kerugian[] =  (float)$data['indeks_kerugianrupiah'];
}
?>
...
<script type="text/javascript">
  ...
  var barChartData = {
    ...
    datasets: [{
      ...
      borderWidth: 1,
      data: <?= json_encode($index_kerugian); ?>,
    }]
  };
  window.onload = function() {
    window.myBar = new Chart(ctx, {
				...
				options: {
					...
					title: {
						display: true,
						text: 'indeks Kerugian Dan Kerusakan Lingkungan'
					}
				}
			});
  };
